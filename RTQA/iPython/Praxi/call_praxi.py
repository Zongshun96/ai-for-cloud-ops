...
def praxi():
    ...
    for length in range(1, 2):
        ...
        for package_names in product(packages_l, packages_l_3):
            for _ in range(2):
                ...
                p2 = subprocess.Popen(['/home/cc/Praxi-study/Praxi-Pipeline/venv/bin/python3', os.path.join(dirname, 'tagset_gen.py'),'-c',os.path.join(out_dirname, labels_str+'-changesets'),'-t',os.path.join(out_dirname, labels_str+'-tagsets')], stdin=subprocess.PIPE, stderr=subprocess.DEVNULL)
