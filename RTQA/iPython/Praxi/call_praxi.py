    ...
    for length in range(1, 1):
        for package_names in combinations(packages_l, length):
            for _ in range(1):
                ...
                cmd1 = "pip install "+packages_str
                cmd2 = "pip uninstall -y " +packages_str
                print(cmd1)
                print(cmd2)
                ...
                p = subprocess.Popen(['python3', os.path.join(dirname, 'cs_rec.py'),'-t',os.path.join(out_dirname),'-l', *package_names], stdin=subprocess.PIPE)
                ...
                p2 = subprocess.Popen(['python3', os.path.join(dirname, 'tagset_gen.py'),'-c',os.path.join(out_dirname, labels_str+'-changesets'),'-t',os.path.join(out_dirname, labels_str+'-tagsets')], stdin=subprocess.PIPE, stderr=subprocess.DEVNULL)
